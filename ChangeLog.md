...
- Added: Get-DesktopEntries
- Added: Get-SwapInfo
- Added: Get-NetworkConnections
- Added: Get-AptSources
- Added: Get-GPUInfo
- Added: Get-SystemErrors
- Added: Test-SystemHealth
- Added: Get-NotificationDaemon
- Added: Get-LibraryVersions
- Updated: Get-LinuxPackages / Update-LinuxPackages to use shared package helpers and skip unavailable managers when Type is all
- Updated: Get-CinnamonSpices to support Type (spices, applets, extensions, all)
- Updated: Get-SystemInfo to support Detail and NeoFetch
- Updated: Get-ComputerInfo as compatibility wrapper over Get-SystemInfo detailed output
- Updated: Get-DefaultBrowser / Get-DefaultAudioPlayer as compatibility wrappers over Get-DefaultApplication
- Updated: Get-AppLaunchers / Get-AutoStartApps as compatibility wrappers over Get-DesktopEntries
- Updated: Get-RootCertificates as compatibility wrapper over Get-Certificates
- Fixed: stale package helper references in inventory and package aggregators
- Fixed: Update-DnfPackages behavior to perform updates instead of listing installed packages
- Fixed: Cinnamon applets path check and spices name filtering

#### PR-ready Summary
- Scope: Consolidated overlapping package, Cinnamon, system info, desktop entry, and default application command paths.
- Additions: Introduced new diagnostics and inventory cmdlets for updates, logs, health, network sockets, swap, GPU, apt sources, notification daemon, and library versions.
- Compatibility: Preserved existing command names as wrappers where practical and aligned docs to the new command model.
- Validation: Added smoke tests in [tests/Consolidation.Smoke.Tests.ps1](./tests/Consolidation.Smoke.Tests.ps1) and [tests/Runtime.Smoke.Tests.ps1](./tests/Runtime.Smoke.Tests.ps1).
